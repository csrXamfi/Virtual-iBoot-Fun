...
- (BOOL)configure {
    ...
    options = [[VZMacOSVirtualMachineStartOptions alloc] init];
    [options _setForceDFU:self.forceDFU];
...
- (BOOL)start {
    /* starting (with options - now using the public API) */
    [vm startWithOptions:options completionHandler:^(NSError *error) {
